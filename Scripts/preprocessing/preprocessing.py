...
purpose_translations = {"None":"None",'Reconduire / aller chercher une personne':'pick_up_a_person',
       ...
       'Autre':'other', 'Repas / collation / café':'cafe', 'ND':'not_available', "Repas / collation /cafÃ©":"cafe", "Retour Ã\xa0 la maison":"returning_home","Travail":"work","DÃ©poser / Ramasser":"drop_off_a_person","Magasinage / Commission":"shops","Ã\x89ducation":"education","SantÃ©":"health","RÃ©union pour le travail":"work"}
...
mode_translations = {"Voiture / Moto":"car", "À pied":"walking", "Transport collectif":"public_transport",
                     ...
                     "Autre":"other", "ND":"not_available", "Ã\x80 pied": "walking", "Autre combinaison":"other",
                    "Transport Collectif":"public_transport","VÃ©lo":"cycling","Automobile et transport collectif":"car",
                    "Autre mode":"other", "Automobile":"car"}
...
class dataPreProcessing:
    ...
    def make_translations(self):
        ...
        """
        if 'loisir' in [col.lower() if col else None for col in self.data['purpose'].unique()]:
            self.data['purpose'] = self.data['purpose'].apply(lambda pur: purpose_translations[pur] if pur else pur)
        if 'transport collectif' in [col.lower() if col else None for col in self.data['mode'].unique()]:
